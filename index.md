---
layout: default
title: About
description: "Daniel Feller, PhD: Director, Data Solutions at Rhino Federated Computing and author of The Health Data Handbook, a free online guide to the data types, standards, and formats behind modern health AI."
---

<div class="hero">
  <img src="{{ '/images/hero.jpeg' | relative_url }}" alt="Daniel Feller at Le Corbusier House, Zurich" class="hero-photo">
  <div class="hero-text">
    <h1>Daniel Feller, PhD</h1>
    <h2>Healthcare Data Science &amp; Federated Computing</h2>
    <p class="tagline">Director, Data Solutions at Rhino Federated Computing<br>PhD in Biomedical Informatics, Columbia University<br>Author of <a href="{{ '/book/' | relative_url }}"><em>{{ site.data.book.title }}</em></a></p>
  </div>
</div>
<p class="hero-caption">At <a href="https://en.wikipedia.org/wiki/Le_Corbusier_House_(Zurich)" target="_blank" rel="noopener">Le Corbusier House</a> in Zurich. The house and its furnishings are designed around the <a href="https://en.wikipedia.org/wiki/Modulor" target="_blank" rel="noopener">Modulor</a> &mdash; an <a href="https://en.wikipedia.org/wiki/Anthropometry" target="_blank" rel="noopener">anthropometric</a> scale of proportions grounded in the human body and the <a href="https://en.wikipedia.org/wiki/Golden_ratio" target="_blank" rel="noopener">golden ratio</a>. Visitors are encouraged to place a red sticker at navel height and a blue sticker at eye level.</p>

<section class="about-content">
  <h2>About</h2>

  <p>I'm Director of Data Solutions at <a href="https://www.rhinohealth.com/" target="_blank" rel="noopener">Rhino Federated Computing</a>, where I build tools for collaborative data science and MLOps. Previously, I built the analytics &amp; data science team at <a href="https://www.rightwayhealthcare.com/" target="_blank" rel="noopener">Rightway Healthcare</a>. I have served as an advisor to several companies including <a href="https://swordhealth.com/" target="_blank" rel="noopener">Sword Health</a>.</p>

  <p>In 2020, I completed my PhD in Biomedical Informatics at <a href="https://www.dbmi.columbia.edu/" target="_blank" rel="noopener">Columbia University</a>, where I worked with physicians to develop algorithms for HIV screening. Before that, I was a researcher at the <a href="https://www.health.ny.gov/diseases/aids/" target="_blank" rel="noopener">NYS Department of Health</a>. My <a href="{{ '/research/' | relative_url }}">published research</a> spans clinical natural language processing, social determinants of health in the EHR, patient-generated data, and HIV care quality.</p>
</section>

{% include book-promo.html %}
