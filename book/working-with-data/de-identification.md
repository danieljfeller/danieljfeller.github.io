...
description: "HIPAA Safe Harbor and Expert Determination, GDPR anonymization and pseudonymization, k-anonymity, and differential privacy."
...
The first of the two GDPR standards for de-identification is anonymization, which  requires that re-identification become practically impossible. This sets an extraordinarily high bar and requires the removal of direct identifiers *and* additional data elements such as rare diagnosis codes, employer names (especially if the company is small). European regulators often reference the data elements in HIPAA's Safe Harbor method, though GDPR requires risk assessment *beyond* any fixed list. The advantage of anonymization is that the data ceases to be subject to GDPR requirements, which are very strict and make data analysis and machine learning very challenging. However, true anonymization is remarkably difficult to achieve in practice. Because GDPR judges anonymization by whether re-identification is *reasonably likely* using any means available to anyone—not just the data holder—regulators have repeatedly found that datasets thought to be anonymous were not, once auxiliary data (voter rolls, public records, other leaked datasets) was taken into account. The practical consequence is that many organizations who believe they are working with anonymized data are, legally speaking, still processing personal data and remain fully subject to GDPR. When the bar is genuinely met, the data escapes GDPR entirely—which is exactly why the standard is set so high.
